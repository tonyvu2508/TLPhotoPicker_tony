...
open class TLPhotosPickerViewController: UIViewController {
    ...
    open var collections = [TLAssetsCollection]()
...
//            print("iPhone 12/ 12Promax / 12Mini ")
    ...
    private func addSubmitButton() {
        ...
        submitButton = UIButton(frame: CGRect(x: size.width - 90.0, y: size.height - CGFloat(distance), width: 70, height: 70))
        submitButton?.setImage(UIImage(named: "100"), for: .normal)
...
extension TLPhotosPickerViewController: UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDataSourcePrefetching {
    ...
    open func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let image = asset.fullResolutionImage {
            fullScreenController.inputs = [ImageSource(image: image)]
        }
...
extension TLPhotosPickerViewController: FullScreenSlideshowViewControllerDelegate, TLPhotoCollectionViewCellDelegate {
    func didClickSubmitButton() {
        dismiss(done: true)
    }
    
    func didTouchSelectedCircle(cell: TLPhotoCollectionViewCell) {
